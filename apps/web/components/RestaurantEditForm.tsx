...
/** The prefilled data an edit form needs (from getMineForEdit / admin.getRestaurant). */
...
/** The field payload passed to the save mutation (resubmit / admin.updateRestaurant). */
export type RestaurantFieldValues = {
  nameAr: string;
  nameEn?: string;
  citySlug: string;
...
/**
 * The shared two-pane restaurant edit form (numbered sections + preview/
 * checklist sidebar), identical to the add form. Field saving is delegated to
 * `onSaveFields` so the same UI backs both the submitter resubmit flow and the
 * admin edit flow; photos are synced through the (admin-capable) submissions
 * upload API afterward.
 */
export function RestaurantEditForm({
  ...
  editable: EditableRestaurant;
  onSaveFields: (values: RestaurantFieldValues) => Promise<unknown>;
  redirectTo: string;
  successTitle: string;
  submitLabel: string;
  submitIcon?: string;
    ...
    >
      ...
      <aside className="mt-6 md:mt-0 md:sticky md:top-20">
        <div className="flex flex-col gap-4 rounded-card border border-line bg-surface p-5">
          ...
          <p className={`${hintClass} text-center`}>{footerNote}</p>
        </div>
      </aside>
    </form>
  );
