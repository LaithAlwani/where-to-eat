...
export function RestaurantCard({ restaurant }: { restaurant: Card }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-card bg-surface shadow-sm ring-1 ring-ink/5 transition duration-200 hover:-translate-y-1 hover:shadow-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 motion-reduce:transition-none motion-reduce:hover:transform-none"
    >
      <div className="relative overflow-hidden">
        ...
        />
        <span className="absolute top-2 inset-s-2 inline-flex items-center gap-1 rounded-pill bg-surface/90 px-2.5 py-1 text-xs font-semibold text-ink shadow-sm backdrop-blur">
      ...
      <div className="flex flex-1 flex-col gap-2 p-4 text-start">
        <div className="flex flex-col gap-0.5">
          <h3 className="font-heading text-lg font-bold text-ink transition group-hover:text-brand-600">
            {restaurant.nameAr}
          </h3>
          {restaurant.nameEn && (
